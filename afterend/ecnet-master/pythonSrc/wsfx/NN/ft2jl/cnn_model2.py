...
class TextCNN(object):
    ...
    def __init__(self, config):
        ...
        # 三个待输入的数据
        self.input_x_1 = tf.placeholder(tf.int32, [None, self.config.seq_length_1], name='input_x_1')
        self.input_x_2 = tf.placeholder(tf.int32, [None, self.config.seq_length_2], name='input_x_2')
    ...
    def cnn(self):
        ...
        with tf.device('/cpu:0'):
            with tf.variable_scope("embedding1"):
                embedding = tf.get_variable('embedding',
                                            [self.config.vocab_size, self.config.embedding_dim])  # 注意，列数是词向量维度
                embedding_inputs_1 = tf.nn.embedding_lookup(embedding,
                                                            self.input_x_1)  # 将根据词典索引格式的input_x映射成[词向量的维数，该段文本的词个数]

        with tf.device('/cpu:0'):
            with tf.variable_scope("embedding2"):
                embedding = tf.get_variable('embedding',
                                            [self.config.vocab_size, self.config.embedding_dim])  # 注意，列数是词向量维度
                embedding_inputs_2 = tf.nn.embedding_lookup(embedding,
                                                            self.input_x_2)  # 将根据词典索引格式的input_x映射成[词向量的维数，该段文本的词个数]
        with tf.name_scope("cnn1"):
            # CNN layer
            with tf.variable_scope("cnn-var1"):
                conv = tf.layers.conv1d(embedding_inputs_1, self.config.num_filters, self.config.kernel_size, name='conv')#(?,26,256)
        ...
        with tf.name_scope("cnn2"):
            ...
            with tf.variable_scope("cnn-var2"):
                conv = tf.layers.conv1d(embedding_inputs_2, self.config.num_filters, self.config.kernel_size,
                                        name='conv')#(?,46,256)
                # global max pooling layer
